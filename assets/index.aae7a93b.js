...
const createUser = (email, password, name2) => {
  createUserWithEmailAndPassword(auth, email, password).then(() => {
    const loggedUser = auth.currentUser;
    loggedUser.getIdToken(true).then(() => {
...
const spanLikeFunc = (docRef) => {
  ...
  if (docRef.data().likes !== void 0) {
    spanLike.innerHTML = `(${docRef.data().likes.length})`;
  }
  likeImg.addEventListener("click", () => {
    if (docRef.likes === void 0) {
      addLike(docRef.id, []);
    } else {
...
const Register = (onNavigate2) => {
  ...
  passwordInput.type = "password";
  passwordInput.id = "myPasswordInput";
  passwordInput.placeholder = "Ingresa tu contrase\xF1a - m\xEDnimo 6 d\xEDgitos";
  passwordInput.minLength = 6;
  passwordInput.required = true;
  ...
  registerBttn.addEventListener("click", (e) => {
    ...
    if (validatePassword(password1, password2) === false) {
      ...
      createUser(email, password1, name2);
      onNavigate2("/timeline");
    }
  });
...
const Timeline = (onNavigate2) => {
  ...
  homeButton.addEventListener("click", () => onNavigate2("/"));
  profileButton.addEventListener("click", () => onNavigate2("/profile"));
  ...
  If(postsRef, (querySnapshot) => {
    ...
    querySnapshot.forEach((post) => {
      ...
      postsDiv.appendChild(createPostDiv(name2, localDate, localTime, content, docId, spanLike));
    });
  });
  return timelineDiv;
};
